...
public class MapperXmlGenerator {
    public static void generate() {
        ...
        ve.init();
        //模板文件所在的classpath的路径
        Template t = ve.getTemplate("/templates/mapper.xml.vm");
        //数据
        ConfigModel configModel = ConfigData.getConfigModel();
        VelocityContext ctx=new VelocityContext();
        ctx.put("config", configModel);

        String outPath = projectPath + "/src/main/resources/mapping/" + configModel.getModulePackage() + "/" + configModel.getEntityName() + "Mapper.xml";
